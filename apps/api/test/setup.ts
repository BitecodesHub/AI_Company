 ...
 * set) so DB-backed integration tests run locally. Walks up from cwd to find it.
 * If no `.env` is found (e.g. CI without a database), DB-backed suites skip
 * gracefully via their own `describe.skipIf(!DATABASE_URL)` guards.
 */
